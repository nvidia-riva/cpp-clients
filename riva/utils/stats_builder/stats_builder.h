...
class StatsBuilder {
    ...
    public:
        StatsBuilder(const std::string& objectName, double audio_duration_seconds, std::size_t num_iterations);
        ...
        void ReportTabularStats() const {
            ...
            double total_audio_processed = audio_duration_seconds_ * performanceStats_.size(); // Total audio across all iterations
            double total_throughput = 0.0;
            
            for (const auto& stats : performanceStats_) {
                if (stats.IsSuccess()) success_count++;
            ...
            std::cout << std::left 
                      << std::setw(15) << "SUMMARY"
                      << std::setw(10) << (success_count == performanceStats_.size() ? "ALL" : std::to_string(success_count) + "/" + std::to_string(performanceStats_.size()))
                      << std::fixed << std::setprecision(3)
                      << std::setw(12) << total_runtime
                      << std::setw(15) << total_audio_processed
