...
PerformanceStats::PerformanceStats(const std::string& objectName)
    : success_(false), objectName_(objectName),
      processing_start_time_(std::chrono::steady_clock::now()),
      processing_end_time_(std::chrono::steady_clock::now()), audio_duration_seconds_(0.0)
{
}

StatsBuilder::StatsBuilder(
    const std::string& objectName, double audio_duration_seconds, std::size_t num_iterations)
    : audio_duration_seconds_(audio_duration_seconds), num_iterations_(num_iterations),
...
void
PerformanceStats::StartProcessingTimer()
{
  processing_start_time_ = std::chrono::steady_clock::now();
  // std::cout << "Starting processing timer: " <<
  // std::chrono::duration_cast<std::chrono::milliseconds>(processing_start_time_.time_since_epoch()).count()
  // << std::endl;
}

void
PerformanceStats::EndProcessingTimer()
{
  processing_end_time_ = std::chrono::steady_clock::now();
  // std::cout << "Ending processing timer: " <<
  // std::chrono::duration_cast<std::chrono::milliseconds>(processing_end_time_.time_since_epoch()).count()
  // << std::endl;
}

double
PerformanceStats::GetRuntimeInMs() const
{
...
double
PerformanceStats::GetRuntimeInSeconds() const
{
  return GetRuntimeInMs() / 1000.0;
}

void
PerformanceStats::SetAudioDurationInSeconds(double audio_duration_seconds)
{
  audio_duration_seconds_ = audio_duration_seconds;
}

double
PerformanceStats::GetThroughputRTFX() const
{
...
void
PerformanceStats::SetObjectName(const std::string& objectName)
{
  objectName_ = objectName;
}

std::string
PerformanceStats::GetObjectName() const
{
  return objectName_;
}

void
PerformanceStats::ReportStats()
{
  ...
  std::cout << "Total Runtime: " << GetRuntimeInMs() << " ms (" << GetRuntimeInSeconds()
            << " seconds)" << std::endl;
...
StatsBuilder::ReportCumulativeStats()
{
  ...
  for (auto performanceStats : performanceStats_) {
    ...
    std::cout << "Total Runtime: " << performanceStats.GetRuntimeInMs() << " ms ("
              << performanceStats.GetRuntimeInSeconds() << " seconds)" << std::endl;
...
double
CalculatePercentile(const std::vector<double>& values, double percentile)
{
  if (values.empty())
    return 0.0;
...
double
StatsBuilder::GetAverageRuntime() const
{
  if (performanceStats_.empty())
    return 0.0;
...
double
StatsBuilder::GetP50Runtime() const
{
...
double
StatsBuilder::GetP90Runtime() const
{
  std::vector<double> runtimes;
  for (const auto& stats : performanceStats_) {
    runtimes.push_back(stats.GetRuntimeInMs());
...
double
StatsBuilder::GetP95Runtime() const
{
  std::vector<double> runtimes;
  for (const auto& stats : performanceStats_) {
    runtimes.push_back(stats.GetRuntimeInMs());
...
double
StatsBuilder::GetP99Runtime() const
{
  std::vector<double> runtimes;
  for (const auto& stats : performanceStats_) {
    runtimes.push_back(stats.GetRuntimeInMs());
...
double
StatsBuilder::GetMinRuntime() const
{
  if (performanceStats_.empty())
    return 0.0;
...
double
StatsBuilder::GetMaxRuntime() const
{
  if (performanceStats_.empty())
    return 0.0;
...
double
StatsBuilder::GetAverageThroughput() const
{
  if (performanceStats_.empty())
    return 0.0;
...
// Statistical methods for throughput
double
StatsBuilder::GetCumulativeThroughput() const
{
  if (performanceStats_.empty())
    return 0.0;
...
double
StatsBuilder::GetP90Throughput() const
{
...
double
StatsBuilder::GetP95Throughput() const
{
  std::vector<double> throughputs;
  for (const auto& stats : performanceStats_) {
    throughputs.push_back(stats.GetThroughputRTFX());
...
double
StatsBuilder::GetP99Throughput() const
{
  std::vector<double> throughputs;
  for (const auto& stats : performanceStats_) {
    throughputs.push_back(stats.GetThroughputRTFX());
...
bool
StatsBuilder::AreAllIterationsSuccessful() const
{
  if (performanceStats_.empty())
    return false;
...
std::size_t
StatsBuilder::GetSuccessfulIterationsCount() const
{
...
std::size_t
StatsBuilder::GetFailedIterationsCount() const
{
  return performanceStats_.size() - GetSuccessfulIterationsCount();
}

double
StatsBuilder::GetSuccessRate() const
{
  if (performanceStats_.empty())
    return 0.0;
...
void
StatsBuilder::ReportDetailedStats() const
{
  ...
  std::cout << "All Iterations Successful: " << (AreAllIterationsSuccessful() ? "YES" : "NO")
            << std::endl;
