...
std::string
nvidia::riva::realtime::RealtimeClient::MakeHttpRequest(
    const std::string& host, int port, const std::string& path, const std::string& method,
...
bool
nvidia::riva::realtime::RealtimeClient::InitializeHttpSession()
{
  ...
  try {
    ...
    if (session_data.HasMember("recognition_config")) {
      ...
      if (recognition.HasMember("enable_automatic_punctuation")) {
        serverDefaults.automatic_punctuation_ =
            recognition["enable_automatic_punctuation"].GetBool();
      }
    ...
    std::cout << "[" << objectName_ << "] Session initialized with defaults: " << buffer.GetString()
              << std::endl;
    return true;
  }
...
nvidia::riva::realtime::RealtimeClient::RealtimeClient(
    const std::string& objectName, const std::shared_ptr<AudioChunks> audioChunksPtr,
    nvidia::riva::utils::PerformanceStats& perfCounter)
    : WebSocketClientBase("ws://127.0.0.1:9090/v1/realtime?intent=transcription"),
      sessionInitialized_(false), sessionUpdated_(false), transcriptionCompleted_(false),
      finalTranscriptionCount_(0), connectionTimeoutInMs_(std::size_t(10000)),
      sessionInitTimeoutInMs_(std::size_t(10000)), sessionUpdateTimeoutInMs_(std::size_t(10000)),
      transcriptionTimeoutInMs_(std::size_t(10000)), chunkDelayTimeInMs_(std::size_t(1000)),
      objectName_(objectName), audioChunksPtr_(audioChunksPtr), perfCounter_(perfCounter)
{
...
void
nvidia::riva::realtime::RealtimeClient::SetTimingConfig(
    const std::size_t connectionTimeoutInMs, const std::size_t sessionInitTimeoutInMs,
    const std::size_t sessionUpdateTimeoutInMs, const std::size_t transcriptionTimeoutInMs,
...
void
nvidia::riva::realtime::RealtimeClient::Log(const std::string& message)
{
  std::cout << "[" << objectName_ << "]" << message << std::endl;
}

bool
nvidia::riva::realtime::RealtimeClient::WaitForTranscriptionCompletion()
{
  ...
  bool completed = transcriptionCv_.wait_for(
      lock, std::chrono::milliseconds(transcriptionTimeoutInMs_),
      [this] { return transcriptionCompleted_; });

  if (!completed) {
    Log(" Timeout waiting for transcription completion after " +
        std::to_string(transcriptionTimeoutInMs_) + " milliseconds");
...
bool
nvidia::riva::realtime::RealtimeClient::WaitForSessionUpdate()
{
  ...
  sessionUpdated_ = sessionCv_.wait_for(
      lock, std::chrono::milliseconds(sessionUpdateTimeoutInMs_),
      [this] { return sessionUpdated_; });

  if (!sessionUpdated_) {
    Log("Timeout waiting for session update after " + std::to_string(sessionUpdateTimeoutInMs_) +
        " milliseconds");
  }
...
void
nvidia::riva::realtime::RealtimeClient::SendAudioAppend(const std::string& audioBase64)
{
...
void
nvidia::riva::realtime::RealtimeClient::SendAudioCommit()
{
  std::lock_guard<std::mutex> lock(connectionMutex_);
  if (IsConnectionOpen()) {
    rapidjson::Document doc;
...
void
nvidia::riva::realtime::RealtimeClient::SendAudioDone()
{
  std::lock_guard<std::mutex> lock(connectionMutex_);
  if (IsConnectionOpen()) {
    rapidjson::Document doc;
...
bool
nvidia::riva::realtime::RealtimeClient::InitializeSession()
{
  std::cout << "[" << objectName_ << "]"
            << " Initializing session..." << std::endl;
...
bool
nvidia::riva::realtime::RealtimeClient::UpdateSessionConfig()
{
  ...
  transcription_config.AddMember(
      "language", rapidjson::Value(sessionConfig_.language_code_.c_str(), allocator), allocator);
  transcription_config.AddMember(
      "model", rapidjson::Value(sessionConfig_.model_name_.c_str(), allocator), allocator);
  ...
  recognition_config.AddMember(
      "enable_automatic_punctuation", sessionConfig_.automatic_punctuation_, allocator);
  recognition_config.AddMember(
      "enable_word_time_offsets", sessionConfig_.word_time_offsets_, allocator);
  recognition_config.AddMember(
      "enable_profanity_filter", sessionConfig_.profanity_filter_, allocator);
  recognition_config.AddMember(
      "enable_verbatim_transcripts", sessionConfig_.verbatim_transcripts_, allocator);
  recognition_config.AddMember(
      "custom_configuration",
      rapidjson::Value(sessionConfig_.custom_configuration_.c_str(), allocator), allocator);
  ...
  diarization_config.AddMember(
      "enable_speaker_diarization", sessionConfig_.speaker_diarization_, allocator);
  diarization_config.AddMember(
      "max_speaker_count", sessionConfig_.diarization_max_speakers_, allocator);
  ...
  update_request.AddMember(
      "type", rapidjson::Value("transcription_session.update", allocator), allocator);
...
// Send audio chunks
void
nvidia::riva::realtime::RealtimeClient::SendAudioChunks(const bool simulateRealtime)
{
  ...
  if (audioChunksPtr_->size() == 0) {
    std::cerr << "No audio chunks to send. Please add audio chunks to the audio chunks pointer."
              << std::endl;
    return;
  }

  std::cout << "Sending audio chunks with " << (simulateRealtime ? "real-time" : "burst")
            << " timing..." << std::endl;
  ...
  for (const std::string& chunk_base64 : *audioChunksPtr_) {
    ...
    if (simulateRealtime) {
      ...
      auto expected_send_time =
          stream_start_time + std::chrono::milliseconds((chunk_index + 1) * chunk_duration_ms);
      ...
      // auto elapsed_ms = std::chrono::duration<double, std::milli>(current_time -
      // stream_start_time).count(); auto expected_ms = (chunk_index + 1) * chunk_duration_ms; auto
      // drift_ms = elapsed_ms - expected_ms;

      // auto wait_ms = std::chrono::duration<double, std::milli>(time_to_wait).count();
      // std::cout << "[" << objectName_ << "] Chunk " << (chunk_index + 1) << "/" <<
      // audioChunksPtr_->size()
      ...
      if ((chunk_index + 1) % 10 == 0 || chunk_index == audioChunksPtr_->size() - 1) {
        // zstd::cout << "[" << objectName_ << "] Sent " << (chunk_index + 1) << "/" <<
        // audioChunksPtr_->size() << " chunks" << std::endl;
      }
    }
...
void
nvidia::riva::realtime::RealtimeClient::HandleMessage(const std::string& message)
{
...
// Public wrapper methods for microphone audio streaming
void
nvidia::riva::realtime::RealtimeClient::SendAudioAppendPublic(const std::string& audioBase64)
{
  SendAudioAppend(audioBase64);
}

void
nvidia::riva::realtime::RealtimeClient::SendAudioCommitPublic()
{
  SendAudioCommit();
}

void
nvidia::riva::realtime::RealtimeClient::SendAudioDonePublic()
{
  SendAudioDone();
}