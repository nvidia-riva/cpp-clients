...
DEFINE_string(
    riva_uri, "ws://127.0.0.1:9090/v1/realtime?intent=transcription", "URI to access riva-server");
...
DEFINE_bool(
    use_ssl, false,
    "Whether to use SSL credentials or not. If ssl_root_cert is specified, this is assumed to be "
    "true");
...
// Microphone configuration (hardcoded like ASR clients)
const int MIC_SAMPLE_RATE = 16000;  // 16kHz
const int MIC_CHANNELS = 1;         // Mono
const int MIC_BIT_DEPTH = 16;       // 16-bit

// Global client pointer for signal handling
std::vector<nvidia::riva::realtime::RealtimeClient*> g_clients;
std::mutex g_clients_mutex;

// Global exit flag for microphone coordination (like ASR clients)
std::atomic<bool> g_request_exit(false);
...
std::string
format_throughput(double value)
{
...
// Helper function to create appropriate audio chunks based on input type
std::shared_ptr<AudioChunks>
CreateAudioChunks(
    const std::string& audio_file, const std::string& audio_device, int chunk_duration_ms)
{
  if (!audio_device.empty()) {
    // Create microphone-based audio chunks
    std::cout << "Creating microphone audio chunks for device: " << audio_device << std::endl;
    std::cout << "Sample rate: " << MIC_SAMPLE_RATE << " Hz, Channels: " << MIC_CHANNELS
              << ", Bit depth: " << MIC_BIT_DEPTH << std::endl;

    auto mic_chunks = std::make_shared<MicrophoneChunks>(
        audio_device, chunk_duration_ms, MIC_SAMPLE_RATE, MIC_CHANNELS, MIC_BIT_DEPTH);

    if (!mic_chunks->Init()) {
      std::cerr << "Failed to initialize microphone audio chunks" << std::endl;
      return nullptr;
    }

    return mic_chunks;
  } else if (!audio_file.empty()) {
    // Create file-based audio chunks
    std::cout << "Creating file audio chunks for: " << audio_file << std::endl;

    auto file_chunks = std::make_shared<FileAudioChunks>(audio_file, chunk_duration_ms);

    if (!file_chunks->Init()) {
      std::cerr << "Failed to initialize file audio chunks" << std::endl;
      return nullptr;
    }
  ...
  std::cerr << "No audio source specified" << std::endl;
  return nullptr;
}
...
client_runner(
    ...
    PerformanceStats& perfCounter, const std::size_t connectionTimeoutInMs,
    const std::size_t sessionInitTimeoutInMs, const std::size_t sessionUpdateTimeoutInMs,
    const std::size_t transcriptionTimeoutInMs, const std::size_t chunkDelayTimeInMs,
    const bool simulateRealtime = false)
{
  nvidia::riva::realtime::RealtimeClient client(
      perfCounter.GetObjectName(), audio_chunks, perfCounter);
  ...
  client.SetTimingConfig(
      connectionTimeoutInMs, sessionInitTimeoutInMs, sessionUpdateTimeoutInMs,
      transcriptionTimeoutInMs, chunkDelayTimeInMs);
  ...
  // For microphone input, we need to start capture before sending chunks
  if (auto mic_chunks = std::dynamic_pointer_cast<MicrophoneChunks>(audio_chunks)) {
    std::cout << "Starting microphone capture..." << std::endl;

    // Ensure microphone is stopped on early exit
    auto mic_cleanup = [mic_chunks]() {
      if (mic_chunks->IsCapturing()) {
        mic_chunks->StopCapture();
        std::cout << "Stopped microphone capture (cleanup)" << std::endl;
      }
    };

    if (!mic_chunks->StartCapture()) {
    ...
    // For microphone: start continuous audio streaming in background thread
    std::cout << "Starting continuous audio streaming..." << std::endl;

    // Start continuous audio streaming in a separate thread (like ASR clients)
    std::thread audio_thread([&client, mic_chunks, simulateRealtime]() {
      // Continuous streaming loop for microphone input
      while (!g_request_exit && mic_chunks->IsCapturing()) {
        // Get the latest audio chunk from microphone
        std::string latest_chunk = mic_chunks->GetLatestChunk();
        if (!latest_chunk.empty()) {
          // Send the audio chunk to the server
          client.SendAudioAppendPublic(latest_chunk);
          client.SendAudioCommitPublic();
        }
      ...
      // Send audio done when streaming ends
      if (!g_request_exit) {
        client.SendAudioDonePublic();
      }
    });

    // Keep microphone running while waiting for transcription completion
    // The microphone will continue capturing until transcription completes or exit is requested
    std::cout << "Microphone is now active. Press Ctrl+C to stop." << std::endl;

    // For microphone input, wait for user interruption or transcription completion
    // The audio thread will continue running until g_request_exit is set
    while (!g_request_exit && mic_chunks->IsCapturing()) {
      std::this_thread::sleep_for(std::chrono::milliseconds(100));
    }

    // Wait for audio transmission to complete
    audio_thread.join();

  } else {
    // For file-based audio, send chunks normally
  ...
  // Step 6.5: Stop microphone capture if it was used (after transcription completes)
  if (auto mic_chunks = std::dynamic_pointer_cast<MicrophoneChunks>(audio_chunks)) {
    // Set exit flag to stop the audio streaming thread
    g_request_exit = true;

    // Stop microphone capture
    mic_chunks->StopCapture();
    std::cout << "Stopped microphone capture" << std::endl;
  }
...
main(int argc, char* argv[])
{
  ...
  str_usage << "           --audio_device=<device_id (such as default, hw:0,0)> " << std::endl;
  ...
  // Note: Microphone configuration is hardcoded (16kHz, mono, 16-bit) like ASR clients
  ...
  // Validate microphone parameters (using hardcoded values like ASR clients)
  if (!FLAGS_audio_device.empty()) {
    // No validation needed since we use hardcoded values
    // MIC_SAMPLE_RATE = 16000, MIC_CHANNELS = 1, MIC_BIT_DEPTH = 16

    // For microphone input, enforce single request and iteration
    if (FLAGS_num_parallel_requests != 1) {
      std::cout << "Warning: num_parallel_requests set to 1 for microphone input" << std::endl;
      FLAGS_num_parallel_requests = 1;
    }
    if (FLAGS_num_iterations != 1) {
      std::cout << "Warning: num_iterations set to 1 for microphone input" << std::endl;
      FLAGS_num_iterations = 1;
    }
    if (FLAGS_simulate_realtime) {
      std::cout << "Warning: simulate_realtime set to false for microphone input" << std::endl;
      FLAGS_simulate_realtime = false;
    }
  }
  ...
  const std::string audio_device = FLAGS_audio_device;
  ...
  // Create appropriate audio chunks based on input type
  const auto audio_chunks = CreateAudioChunks(audio_file_path, audio_device, chunk_duration_ms);
  if (!audio_chunks) {
    std::cerr << "Failed to create audio chunks" << std::endl;
    return 1;
  }

  PerformanceStats overallPerf("Overall");

  // For microphone input, duration is ongoing, so we'll use a reasonable estimate
  double audio_duration = audio_chunks->GetDurationSeconds();
  if (audio_duration <= 0.0 && !audio_device.empty()) {
    // Microphone input - use a reasonable duration estimate for stats
    audio_duration = 60.0;  // Assume 1 minute for microphone sessions
    std::cout << "Using estimated duration of " << audio_duration << " seconds for microphone input"
              << std::endl;
  }

  overallPerf.SetAudioDurationInSeconds(audio_duration * num_iterations * num_parallel_clients);

  // Set up signal handlers for graceful shutdown (before starting async operations)
  signal(SIGINT, signal_handler);
  signal(SIGTERM, signal_handler);

  // Create StatsBuilder for all clients
  StatsBuilder statsBuilder("client", audio_duration, num_parallel_clients);
  ...
  for (std::size_t N = 0; N < num_parallel_clients; ++N) {
    ...
    futures.emplace_back(std::async(std::launch::async, [&, N]() {
      ...
      for (std::size_t M = 0; M < num_iterations; ++M) {
        ...
        client_runner(
            uri, audio_chunks, statsBuilder.GetPerformanceStats(N), connectionTimeoutInMs,
            sessionInitTimeoutInMs, sessionUpdateTimeoutInMs, transcriptionTimeoutInMs,
