...
nvidia::riva::realtime::WebSocketClientBase::WebSocketClientBase(const std::string& uri)
    : connected_(false), 
      ...
      connectionTimeoutMs_(std::size_t(5000)),
      uri_(uri) {
    ...
    wsClient_.set_close_handler(
        std::bind(&nvidia::riva::realtime::WebSocketClientBase::OnClose, this, std::placeholders::_1));
    ...
    wsClient_.set_message_handler(
        std::bind(&nvidia::riva::realtime::WebSocketClientBase::OnMessage, this, std::placeholders::_1, std::placeholders::_2));
}

void nvidia::riva::realtime::WebSocketClientBase::SetConnectionTimeout(const std::size_t connectionTimeoutMs) {
    connectionTimeoutMs_ = connectionTimeoutMs;
}

std::size_t nvidia::riva::realtime::WebSocketClientBase::GetConnectionTimeout() {
    return connectionTimeoutMs_;
}

void nvidia::riva::realtime::WebSocketClientBase::SetVerboseLogging(bool verbose) {
    if (verbose) {
...
void nvidia::riva::realtime::WebSocketClientBase::Connect(const std::string& uri) {
    uri_ = uri;
...
void nvidia::riva::realtime::WebSocketClientBase::Run() {
    wsClient_.run();
}

void nvidia::riva::realtime::WebSocketClientBase::Send(const std::string& message) {
...
void nvidia::riva::realtime::WebSocketClientBase::Close() {
    std::lock_guard<std::mutex> lock(mutex_);
    if (connected_) {
        websocketpp::lib::error_code ec;
...
void nvidia::riva::realtime::WebSocketClientBase::SendJsonMessage(const std::string& type, const std::string& data) {
    std::lock_guard<std::mutex> lock(mutex_);
    if (connected_) {
...
void nvidia::riva::realtime::WebSocketClientBase::OnOpen(websocketpp::connection_hdl hdl) {
    std::lock_guard<std::mutex> lock(mutex_);
    connectionHdl_ = hdl;
    connected_ = true;
...
void nvidia::riva::realtime::WebSocketClientBase::OnClose(websocketpp::connection_hdl hdl) {
...
void nvidia::riva::realtime::WebSocketClientBase::OnFail(websocketpp::connection_hdl hdl) {
    (void)hdl;  // Suppress unused parameter warning
    std::lock_guard<std::mutex> lock(mutex_);
    connected_ = false;
...
void nvidia::riva::realtime::WebSocketClientBase::OnMessage(websocketpp::connection_hdl hdl, message_ptr msg) {
    (void)hdl;  // Suppress unused parameter warning
    HandleMessage(msg->get_payload());
}

bool nvidia::riva::realtime::WebSocketClientBase::WaitForConnection() {
    std::unique_lock<std::mutex> lock(connectionMutex_);
    return connectionCv_.wait_for(lock, 
        std::chrono::milliseconds(connectionTimeoutMs_),  // Use the provided timeout
        [this] { return connected_; });
}

bool nvidia::riva::realtime::WebSocketClientBase::WaitForDisconnection() {
    std::unique_lock<std::mutex> lock(connectionMutex_);
    return connectionCv_.wait_for(lock, 
        std::chrono::milliseconds(connectionTimeoutMs_),  // Use the provided timeout
        [this] { return !connected_; });
...
bool nvidia::riva::realtime::WebSocketClientBase::WaitForServerClose() {
    std::unique_lock<std::mutex> lock(connectionMutex_);
    return connectionCv_.wait_for(lock, 
        std::chrono::milliseconds(connectionTimeoutMs_),  // Use the provided timeout
